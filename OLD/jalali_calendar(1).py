...
def gregorian_iso_to_jalali_display(iso_date):
    ...
    return (
        ...
        f"{to_persian_digits(f'{jalali.month:02d}')}/"
        f"{to_persian_digits(f'{jalali.day:02d}')}"
    )
...
def open_jalali_date_picker(
    ...
    def choose_date(year, month, day):
        ...
        jalali_display = (
            ...
            f"{to_persian_digits(f'{month:02d}')}/"
            f"{to_persian_digits(f'{day:02d}')}"
        )
        popup.dismiss()
    ...
    def render_month():
        ...
        month_label.text = shape_persian(
            f"{PERSIAN_MONTHS[month]} {to_persian_digits(year)}"
        )
        ...
        for day_number in range(1, _jalali_month_days(year, month) + 1):
            ...
            day_button = Button(
                text=to_persian_digits(day_number),
                font_name="Persian",
                font_size=16,
                background_normal="",
                background_down="",
