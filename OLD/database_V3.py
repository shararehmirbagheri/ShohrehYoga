...
class Database:
    ...
    def save_attendance_for_date(self, attended_on, present_student_ids):
        """
        Replace all attendance records for one date.

        Checked students are stored as present.
        Unchecked students are treated as absent and are not stored.
        """
        self.cursor.execute(
        ...
        student_ids = sorted({int(student_id) for student_id in present_student_ids})
        self.cursor.executemany(
            """
            INSERT INTO attendances(student_id, date)
            VALUES (?, ?)
            """,
            [(student_id, attended_on) for student_id in student_ids],
        )
        self.conn.commit()
